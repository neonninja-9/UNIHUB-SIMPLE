...
const AttendanceUploader: React.FC<AttendanceUploaderProps> = ({ students, onMarkAttendance }) => {
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (students.length === 0) {
      setModalMessage('No registered students found. Please capture student faces before marking attendance.');
      setShowModal(true);
      inputElement.value = '';
      return;
    }
    ...
    try {
      ...
      try {
        if (attendanceRecords.length > 0) {
          await Promise.all(
            attendanceRecords.map(async record => {
              ...
              if (!response.ok) {
                throw new Error(`Failed to record attendance for student ${record.studentId}`);
              }
            })
          );
        }
        ...
        const successMessage = attendanceRecords.length > 0
