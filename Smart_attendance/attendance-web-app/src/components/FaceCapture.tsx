  ...
  const captureFace = async () => {
    ...
    try {
      ...
      if (detections.length === 0) {
        setModalMessage('No face detected. Please position yourself in front of the camera.');
        setShowModal(true);
        return;
      }
      if (detections.length > 1) {
        setModalMessage('Multiple faces detected. Please ensure only one face is visible.');
        setShowModal(true);
        return;
      }
