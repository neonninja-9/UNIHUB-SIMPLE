        ...
        <div className="card-front absolute w-full h-full [backface-visibility:hidden] rounded-2xl overflow-hidden shadow-2xl bg-white flex flex-col">
            <div className="bg-[#0f2e5f] text-white p-4 flex items-center justify-between">
                <div className="flex items-center space-x-3">
                    ...
                    <div>
                        <h1 className="font-bold text-xl tracking-wide">{student.universityName}</h1>
                        <p className="text-xs font-light tracking-widest border-t border-yellow-400 pt-1 mt-1">{student.universityCampus}</p>
                    </div>
                </div>
            </div>
            <div className="bg-[#e88f4b] flex-grow flex flex-col items-center justify-center p-4 relative">
                <div className="absolute top-0 left-0 w-full h-full bg-black opacity-5"></div>
                 <div className="w-36 h-44 border-4 border-white rounded-lg overflow-hidden shadow-lg mb-3">
                    <img src={student.studentPhotoUrl} alt="Student photo" className="w-full h-full object-cover" />
                </div>
                <h2 className="text-2xl font-bold text-gray-800">{student.studentName}</h2>
                <p className="text-lg font-medium text-gray-700">{student.degree} ({student.branch})</p>
                <p className="text-md font-normal text-gray-600 mt-1">{student.session}</p>

                <div className="absolute bottom-4 w-full px-6 flex justify-between items-end">
                    <div className="text-center">
                        <img src={student.issuingAuthoritySignatureUrl} alt="Issuing Authority Signature" className="h-8 w-24 mx-auto"/>
                        <p className="text-xs font-semibold text-gray-800 border-t border-gray-600 pt-1 mt-1">Issuing Authority</p>
                    </div>
                    <div className="text-center">
                        <img src={student.holderSignatureUrl} alt="Holder's Signature" className="h-8 w-24 mx-auto"/>
                        <p className="text-xs font-semibold text-gray-800 border-t border-gray-600 pt-1 mt-1">Holder's Signature</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="card-back absolute w-full h-full [backface-visibility:hidden] [transform:rotateY(180deg)] rounded-2xl overflow-hidden shadow-2xl bg-white p-6 text-gray-800 flex flex-col justify-between font-mono">
            ...
            <div className="my-4">
                ...
                <ul className="list-none pl-2 mt-1 space-y-1">
                    <li><span className="font-semibold">■ Family :</span> {student.emergencyContact.family}</li>
                    <li><span className="font-semibold">■ Institution :</span> {student.emergencyContact.institution}</li>
                </ul>
            </div>
            ...
            <div className="border-t border-gray-200 mt-auto pt-3 text-center text-xs">
                ...
                <p>the office number - <span className="font-semibold">{student.lostAndFoundContact}</span></p>
            </div>
        </div>
      </div>
    </div>
